...
import { ManagerDrinkTypeStyle } from '../../styles/LayoutStyle';
import { SafeAreaProvider } from 'react-native-safe-area-context';
import { createDrawerNavigator, DrawerContentScrollView, DrawerItemList, DrawerItem, } from '@react-navigation/drawer';
import { host, port, drinkTypeSearchUrl, drinkTypeListUrl, drinkTypeAddUrl, drinkTypeDeleteUrl, drinkTypeUpdateUrl } from '../../apis/ManagerApi';
...
class ManagerDrinkTypeScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      searchModal: false,
    ...
    return (
      <View style={{ flex: 1 }}>
        <Modal
          isVisible={isVisible}
        >
          <View style={ManagerDrinkTypeStyle.searchContainer}>
  ...
  searchItem(value) {
    ...
    if (this.state.searchModalInitial == 0) {
      fetch(host + ':' + port + drinkTypeListUrl + value, {
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Bearer ' + this.props.user.token,
        }
    ...
    if (this.state.searchModalInitial == 1) {
      fetch(host + ':' + port + drinkTypeSearchUrl + 'name=' + value, {
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Bearer ' + this.props.user.token,
        }
  ...
  deleteItem = (id) => {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: "Đồng ý", onPress: () => {
            fetch(host + ':' + port + drinkTypeDeleteUrl + id, {
              method: 'DELETE',
              headers: {
              ...
              .then(data => {
                if (data.error == 'Internal Server Error') {
                  Alert.alert(
                    "Thông báo",
                    "Không thể xóa loại đồ uống vì lý do bảo toàn dữ liệu",
                    [
                      {
                        text: "Đóng",
  ...
  addItemView = () => {
    return (
      <Animated.View
        style={[
          ManagerDrinkTypeStyle.container,
          {
      ...
      >
        ...
        <SafeAreaProvider>
          <View style={ManagerDrinkTypeStyle.setupItem}>
            <Text style={ManagerDrinkTypeStyle.title}>Thêm nhà kho mới: </Text>
            <View style={ManagerDrinkTypeStyle.setupItemCenterContainer}><Text style={ManagerDrinkTypeStyle.error}>{this.state.error}</Text></View>
            <Input
            ...
            <Input
              ...
              keyboardType='default' />
            <View style={ManagerDrinkTypeStyle.setupItemCenterContainer}>
              <TouchableOpacity onPress={this.saveAddItem}>
                <Image
                  style={ManagerDrinkTypeStyle.saveButton}
              ...
              <TouchableOpacity onPress={this.cancelItem}>
                <Image
                  style={ManagerDrinkTypeStyle.cancelButton}
  ...
  updateItemView = () => {
    return (
      ...
      >
        <SafeAreaProvider>
          <View style={ManagerDrinkTypeStyle.setupItem}>
            <Text style={ManagerDrinkTypeStyle.title}>Cập nhật nhà kho: {this.state.id}</Text>
            <View style={ManagerDrinkTypeStyle.setupItemCenterContainer}><Text style={ManagerDrinkTypeStyle.error}>{this.state.error}</Text></View>
            <Input
              inputStyle={{ fontSize: this.state.windowWidth / 28 }}
              inputContainerStyle={{ borderRadius: 10, marginTop: '2%', backgroundColor: 'white' }}
  ...
  renderItem = ({ item }) => (
    <View>
      <View style={ManagerDrinkTypeStyle.item}>
        <View>
          <TouchableOpacity onPress={() => { this.deleteItem(item.id); }}>
            <Icon
              style={ManagerDrinkTypeStyle.icon}
              name='trash-outline'
              type='ionicon'
              color='red'
            />
        ...
        <View><Image
          style={ManagerDrinkTypeStyle.logo}
  ...
  saveAddItem = () => {
    ...
    fetch(host + ':' + port + drinkTypeAddUrl, {
      method: 'POST',
      headers: {
  ...
  saveUpdateItem = () => {
    ...
    fetch(host + ':' + port + drinkTypeUpdateUrl + this.state.id, {
      method: 'PUT',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
  ...
  render() {
    return (
      <View style={ManagerDrinkTypeStyle.container}>
          ...
          <View style={ManagerDrinkTypeStyle.container}>
            <Animated.View
              ...
              <SafeAreaProvider>
                ...
                <View style={ManagerDrinkTypeStyle.topcontainer}>
                ...
                <View style={ManagerDrinkTypeStyle.centercontainer}>
                  <Text style={ManagerDrinkTypeStyle.title}>Danh sách:</Text>
                  {this.state.flag ?
                    <View>
              ...
              <View style={ManagerDrinkTypeStyle.plusContainer}>
                <TouchableOpacity onPress={this.addItem}>

                  <Image
                    style={ManagerDrinkTypeStyle.plus}
...
export default connect(mapStateToProps)(ManagerDrinkTypeScreen);