...
import { ManagerAreaStyle } from '../../styles/LayoutStyle';
import { SafeAreaProvider } from 'react-native-safe-area-context';
import { createDrawerNavigator, DrawerContentScrollView, DrawerItemList, DrawerItem, } from '@react-navigation/drawer';
import { host, port, areaSearchUrl, areaListUrl, areaAddUrl, areaDeleteUrl, areaUpdateUrl } from '../../apis/ManagerApi';
...
class ManagerAreaScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      searchModal: false,
    ...
    return (
      <View style={{ flex: 1 }}>
        <Modal
          isVisible={isVisible}
        >
          <View style={ManagerAreaStyle.searchContainer}>
  ...
  searchItem(value) {
    ...
    if (this.state.searchModalInitial == 0) {
      fetch(host + ':' + port + areaListUrl + value, {
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Bearer ' + this.props.user.token,
        }
    ...
    if (this.state.searchModalInitial == 1) {
      fetch(host + ':' + port + areaSearchUrl + 'name=' + value, {
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Bearer ' + this.props.user.token,
        }
  ...
  deleteItem = (id) => {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: "Đồng ý", onPress: () => {
            fetch(host + ':' + port + areaDeleteUrl + id, {
              method: 'DELETE',
              headers: {
  ...
  addItemView = () => {
    return (
      ...
      >
        ...
        <SafeAreaProvider>
          <View style={ManagerAreaStyle.setupItem}>
            <Text style={ManagerAreaStyle.title}>Thêm nhà kho mới: </Text>
            <View style={ManagerAreaStyle.setupItemCenterContainer}><Text style={ManagerAreaStyle.error}>{this.state.error}</Text></View>
            <Input
            ...
            <Input
              ...
              keyboardType='default' />
            <View style={ManagerAreaStyle.setupItemCenterContainer}>
              <TouchableOpacity onPress={this.saveAddItem}>
                <Image
                  style={ManagerAreaStyle.saveButton}
              ...
              <TouchableOpacity onPress={this.cancelItem}>
                <Image
                  style={ManagerAreaStyle.cancelButton}
  ...
  updateItemView = () => {
    return (
      ...
      >
        <SafeAreaProvider>
          <View style={ManagerAreaStyle.setupItem}>
            <Text style={ManagerAreaStyle.title}>Cập nhật nhà kho: {this.state.id}</Text>
            <View style={ManagerAreaStyle.setupItemCenterContainer}><Text style={ManagerAreaStyle.error}>{this.state.error}</Text></View>
            <Input
              inputStyle={{ fontSize: this.state.windowWidth / 28 }}
              inputContainerStyle={{ borderRadius: 10, marginTop: '2%', backgroundColor: 'white' }}
  ...
  renderItem = ({ item }) => (
    <View>
      <View style={ManagerAreaStyle.item}>
        <View>
          <TouchableOpacity onPress={() => { this.deleteItem(item.id); }}>
            <Icon
              style={ManagerAreaStyle.icon}
              name='trash-outline'
              type='ionicon'
              color='red'
            />
  ...
  saveUpdateItem = () => {
    ...
    fetch(host + ':' + port + areaUpdateUrl + this.state.id, {
      method: 'PUT',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
  ...
  render() {
    return (
      <View style={ManagerAreaStyle.container}>
          ...
          <View style={ManagerAreaStyle.container}>
            <Animated.View
              ...
              <SafeAreaProvider>
                ...
                <View style={ManagerAreaStyle.topcontainer}>
                ...
                <View style={ManagerAreaStyle.centercontainer}>
                  <Text style={ManagerAreaStyle.title}>Danh sách:</Text>
                  {this.state.flag ?
                    <View>
              ...
              <View style={ManagerAreaStyle.plusContainer}>
                <TouchableOpacity onPress={this.addItem}>

                  <Image
                    style={ManagerAreaStyle.plus}
...
export default connect(mapStateToProps)(ManagerAreaScreen);