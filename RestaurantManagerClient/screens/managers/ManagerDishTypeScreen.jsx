...
import { ManagerDishTypeStyle } from '../../styles/LayoutStyle';
import { SafeAreaProvider } from 'react-native-safe-area-context';
import { createDrawerNavigator, DrawerContentScrollView, DrawerItemList, DrawerItem, } from '@react-navigation/drawer';
import { host, port, dishTypeSearchUrl, dishTypeListUrl, dishTypeAddUrl, dishTypeDeleteUrl, dishTypeUpdateUrl } from '../../apis/ManagerApi';
...
class ManagerDishTypeScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      searchModal: false,
    ...
    return (
      <View style={{ flex: 1 }}>
        <Modal
          isVisible={isVisible}
        >
          <View style={ManagerDishTypeStyle.searchContainer}>
  ...
  searchItem(value) {
    ...
    if (this.state.searchModalInitial == 0) {
      fetch(host + ':' + port + dishTypeListUrl + value, {
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Bearer ' + this.props.user.token,
        }
    ...
    if (this.state.searchModalInitial == 1) {
      fetch(host + ':' + port + dishTypeSearchUrl + 'name=' + value, {
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Bearer ' + this.props.user.token,
        }
  ...
  deleteItem = (id) => {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: "Đồng ý", onPress: () => {
            fetch(host + ':' + port + dishTypeDeleteUrl + id, {
              method: 'DELETE',
              headers: {
              ...
              .then(data => {
                if (data.error == 'Internal Server Error') {
                  Alert.alert(
                    "Thông báo",
                    "Không thể xóa loại món ăn vì lý do bảo toàn dữ liệu",
                    [
                      {
                        text: "Đóng",
  ...
  addItemView = () => {
    return (
      <Animated.View
        style={[
          ManagerDishTypeStyle.container,
          {
      ...
      >
        ...
        <SafeAreaProvider>
          <View style={ManagerDishTypeStyle.setupItem}>
            <Text style={ManagerDishTypeStyle.title}>Thêm nhà kho mới: </Text>
            <View style={ManagerDishTypeStyle.setupItemCenterContainer}><Text style={ManagerDishTypeStyle.error}>{this.state.error}</Text></View>
            <Input
            ...
            <Input
              ...
              keyboardType='default' />
            <View style={ManagerDishTypeStyle.setupItemCenterContainer}>
              <TouchableOpacity onPress={this.saveAddItem}>
                <Image
                  style={ManagerDishTypeStyle.saveButton}
              ...
              <TouchableOpacity onPress={this.cancelItem}>
                <Image
                  style={ManagerDishTypeStyle.cancelButton}
  ...
  updateItemView = () => {
    return (
      ...
      >
        <SafeAreaProvider>
          <View style={ManagerDishTypeStyle.setupItem}>
            <Text style={ManagerDishTypeStyle.title}>Cập nhật nhà kho: {this.state.id}</Text>
            <View style={ManagerDishTypeStyle.setupItemCenterContainer}><Text style={ManagerDishTypeStyle.error}>{this.state.error}</Text></View>
            <Input
              inputStyle={{ fontSize: this.state.windowWidth / 28 }}
              inputContainerStyle={{ borderRadius: 10, marginTop: '2%', backgroundColor: 'white' }}
  ...
  renderItem = ({ item }) => (
    <View>
      <View style={ManagerDishTypeStyle.item}>
        <View>
          <TouchableOpacity onPress={() => { this.deleteItem(item.id); }}>
            <Icon
              style={ManagerDishTypeStyle.icon}
              name='trash-outline'
              type='ionicon'
              color='red'
            />
        ...
        <View><Image
          style={ManagerDishTypeStyle.logo}
  ...
  saveAddItem = () => {
    ...
    fetch(host + ':' + port + dishTypeAddUrl, {
      method: 'POST',
      headers: {
  ...
  saveUpdateItem = () => {
    ...
    fetch(host + ':' + port + dishTypeUpdateUrl + this.state.id, {
      method: 'PUT',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
  ...
  render() {
    return (
      <View style={ManagerDishTypeStyle.container}>
          ...
          <View style={ManagerDishTypeStyle.container}>
            <Animated.View
              ...
              <SafeAreaProvider>
                ...
                <View style={ManagerDishTypeStyle.topcontainer}>
                ...
                <View style={ManagerDishTypeStyle.centercontainer}>
                  <Text style={ManagerDishTypeStyle.title}>Danh sách:</Text>
                  {this.state.flag ?
                    <View>
              ...
              <View style={ManagerDishTypeStyle.plusContainer}>
                <TouchableOpacity onPress={this.addItem}>

                  <Image
                    style={ManagerDishTypeStyle.plus}
...
export default connect(mapStateToProps)(ManagerDishTypeScreen);